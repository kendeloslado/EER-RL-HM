...
while(op_nodes>0 && rnd<tot_rnd)
    ...
    % j represents the cluster members
        ...
        % length simply gets the number of members in a cluster formation
            ...
            % cluster picks the cluster head as next hop (i think)
                if cluster(i,j).E>0
                % node should be alive
                    maxQ = max([cluster(i,:).Q]);
                    if (cluster(i,j).dts_ch<=range_C || cluster(i,j).Q == maxQ)
                    % cluster member must be within range AND/OR cluster's
                    % cluster can also be the one with highest Q-value
                    % Q-value is the max Q-value
                    ...
                    % cluster member is not within range nor maxQ
                        ...
                        if cluster(i,j).prev==0
                        % cluster member has not received packets yet
    ...
    for l=1:n
        if NET(l).E>0
            if NET(l).dts<=range_C && NET(l).role==0
            % checks that it's not a cluster head, and is within range to
            % the sink
                if NET(l).prev==0
                % no packets received
